...
for i in range(1,81):
	...
	'''LSB text to image (key, pathOriginalImage, pathChiperText, pathDestinationImage)'''
	Tti.encrypt(key, "./image/source/LL_haar_2.bmp", "./text/encrypt/"+ str(i) + "_encrypt.txt", "./image/embed/"+ str(i) + "_embed.bmp")
	...
	im1 = cv2.imread("./image/source/LL_haar_2.bmp")
	im2 = cv2.imread("./image/embed/"+ str(i) + "_embed.bmp")

	file = open("./test/size/img_source.txt","a+")
	file.write(str(os.path.getsize("./image/source/LL_haar_2.bmp")))
	file.write("\n")
	file.close()
